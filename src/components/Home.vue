<template>
  <div id="show_blog">
    ...
    <el-input type="text" placeholder="按标题或内容搜索" v-model="search" style="width: 600px;"
              prefix-icon="el-icon-search"></el-input>

    <div>
      <el-checkbox-group v-model="checkedTypes" fill="RGB(62,213,200)">
        <el-checkbox-button v-for="type in types" :label="type" :key="type">{{type}}</el-checkbox-button>
      </el-checkbox-group>
    </div>

    <div class="act" v-for="(blog,index) in filterblogs "
         style="width: 900px;text-align: left; margin:20px auto;background-color: ivory; border-radius: 20px 0 20px 0;overflow: hidden;">
      <router-link :to="/blog/+index" tag="p" style="cursor:pointer">
        <b><p v-color style="text-indent: 30px;font-size: 28px;padding: 30px 20px 0 20px ;padding-bottom: 10px;">
          {{blog.title | upcase}}</p></b>
        <p style="font-size: 25px;padding: 0 50px 40px 50px; margin-bottom: 20px; white-space: pre-line;">
          {{blog.content| pass}}</p>
      </router-link>
